...
@implementation ZMWorrdChatFrameeModel
...
- (instancetype)initWithFrameModel:(ZMChatModell *)modelData timeIsEqual:(BOOL)timeBool{
    ...
    if (self=[super init]) {
        ...
        if(modelData.person){ //如果是男的发送的话,头像就在右边,文字也在左边,否则头像就在左边,文字也在右边
            ...
            //self.btnFrame=CGRectMake(iphoneW-distance-imageH-distance-btnRect.size.width-20*2 + 60, CGRectGetMaxY(self.timeFrame)+ distance - 15, btnRect.size.width+20*2, (btnRect.size.height+20*2) - 20);
            ...
            //self.btnFrame=CGRectMake(distance+imageH+distance - 60, CGRectGetMaxY(self.timeFrame)+distance - 10, btnRect.size.width+20*2, (btnRect.size.height+20*2) - 20);
