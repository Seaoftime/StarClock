...
- (void)drawRect:(CGRect)rect
{
    ...
    if (_type == NZProgressTypeDefault) {
        [self drawBackground:contextRef Rect:CGRectMake(0, 0, rect.size.width-0, rect.size.height)];
    }
    else
    {
    ...
    if (_progress) {
        //绘制进度条
        if (_type == NZProgressTypeDefault) {
            [self drawProgress:contextRef WithRect:CGRectMake(0, 0, rect.size.width-0, rect.size.height)];
        }
        else
        {
...
- (void)drawBackground:(CGContextRef)context Rect:(CGRect)rect
{
    if (_type == NZProgressTypeDefault) {
        ...
        CGContextSetFillColorWithColor(context, RGB(30, 51, 78).CGColor);
        [bgPath fill];
