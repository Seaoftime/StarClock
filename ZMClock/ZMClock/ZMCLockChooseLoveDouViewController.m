//
//  ZMCLockChooseLoveDouViewController.m
//  ZMClock
//
...
@interface ZMCLockChooseLoveDouViewController ()<UITableViewDelegate,UITableViewDataSource>
...
//////////
//@property (nonatomic, copy) NSString *notifyDownloaddStt;

@end

@implementation ZMCLockChooseLoveDouViewController
...
#pragma mark ----------------------------下载铃声的通知
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(downnloaddClockkRingMusicc) name:@"DOWNLOAD_CLOCK_RING_MUSIIIC" object:nil];
...
#pragma mark ---------------------------下载铃声
- (void)downnloaddClockkRingMusicc
{
    
    //////通知下载的 url
    NSString *urlStr = [NSUserDefaults standardUserDefaults].notifyDownloaddStt;
    
    NSString *urlPathh = [NSUserDefaults standardUserDefaults].notifyDnloadPaath;
    
    
    if (urlStr == nil || urlStr == NULL || [urlStr isKindOfClass:[NSNull class]] || urlPathh == nil || urlPathh == NULL || [urlPathh isKindOfClass:[NSNull class]]) {
        ...
        //添加下载任务
         ...
         [[FGGDownloadManager shredManager] downloadWithUrlString:urlStr toPath:urlPathh process:^(float progress, NSString *sizeString, NSString *speedString) {
             //
             
             NSLog(@"下载路径___%@",urlPathh);
...
//                        //[NSUserDefaults standardUserDefaults].clockListHeadPoritStr = encodeImgStr;
//                        [NSUserDefaults standardUserDefaults].clockListHeadPoritStr = model.clocklist_pic;
//                        
//                        
//                        
