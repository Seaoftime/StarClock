//
//  ZMStoryComicNewsInfoViewController.m
//  ZMClock
//
...
@interface ZMStoryInfoViewController () <UITableViewDelegate,UITableViewDataSource,KeyBoardToolBarDeletage,StoryComicNewsInfoViewTableViewCellDeletage>

@property (nonatomic,strong) UITableView *storyInfoTableView;
...
@implementation ZMStoryInfoViewController
...
-(void)starGetNewAndStoryAndComicInfoWithDetailId:(NSString*)detailId
{
    ...
    [self getNewAndStoryAndComicInfoWithCategoryType:@"2" withEndseq:@"0" withActionUpdateHis:@"1" withActionTop:@"1"];
}
...
#pragma mark  设置标题数据
                         ...
                         [self.storyArray addObject:titleFrame];
...
#pragma mark 设置内容数据
                         ...
                         [self.storyArray addObject:contentFrame];
...
#pragma mark 设置图片数据
                        NSArray *imageArray = [contentDictionary objectForKey:@"image_list"];
                         ...
                         for (int i = 0; i< [imageArray count]; i++)
                         {
                             ...
                             [self.storyArray addObject:imageFrame];
...
#pragma mark 设置作者数据
                         ...
                         [self.storyArray addObject:authorFrame];
                     }
...
#pragma mark 设置评论数据
                     ...
                     if ([commentArray count])
                     {
                         for (int i=0; i<[commentArray count]; i++)
                         {
                             ...
                             [self.storyArray addObject:frame];
                         }
                         
                         [self.storyInfoTableView.footer endRefreshing];
                         
                     }
                     else
                     {
                         [self.storyInfoTableView.footer noticeNoMoreData];
                     }
                     [self.storyInfoTableView reloadData];
                 }
                 else
                 {
                     [self.storyInfoTableView.footer noticeNoMoreData];
                ...
                [[AFNetworkingHelper sharedNetworkingHelper] postWithUrl:@"article/content_comment.php" theParmeters:parmeters sucess:^(id object)
                 {
                     if ([[MainViewControllerHelper sharedMainHelper] statusWith:object])
                     {
                         ...
                         if ([itemData count])
                         {
                             ...
                             [self.storyArray enumerateObjectsUsingBlock:^(StoryComicNewsInfoFrame  *_Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop)
                              {
                                  switch (obj.model.type)
                                  {
                                      ...
                                      case authorType:
                                      {
                                          ...
                                          [self.storyArray insertObject:frame atIndex:(idx+1)];
                                          *stop = YES;
                                      }
                                          break;
                                      case titleType:
...
-(void)viewDidLoad
{
    ...
    [self.view addSubview:self.storyInfoTableView];
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    switch (self.editType)
    {
        case 0:
        {
            ...
            [self.storyInfoTableView setFrame:CGRectMake(0, 60, kScreen_Width, kScreen_Height-60)];
        }
            break;
        case 1:
        {
            [self.storyInfoTableView setFrame:CGRectMake(0, 60, kScreen_Width, kScreen_Height-100)];
...
-(UITableView *)storyInfoTableView
{
    if (_storyInfoTableView == nil)
    {
        _storyInfoTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 60, kScreen_Width, kScreen_Height-60) style:UITableViewStylePlain];
        [_storyInfoTableView setSeparatorStyle:UITableViewCellSeparatorStyleNone];
        [_storyInfoTableView setDelegate:self];
        [_storyInfoTableView setDataSource:self];
        [_storyInfoTableView setFooter:[MJRefreshAutoNormalFooter footerWithRefreshingTarget:self refreshingAction:@selector(refreshFooterAction)]];
    }
    return _storyInfoTableView;
}
...
-(KeyBoardToolBar*)keyBoardToolBar
{
    ...
    {
        _keyBoardToolBar = [[KeyBoardToolBar alloc]initWithFrame:CGRectMake(0, kScreen_Height-40, kScreen_Width, 40) withDeletage:self setTableView:self.storyInfoTableView setArray:self.storyArray];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell setInfoframe:self.storyArray[indexPath.row]];
    return cell;
}
...
-(void)sendMessageButtonAction:(UIButton *)sender
{
    ...
    [self submitCommentWithContent:self.keyBoardToolBar.inputTextField.text withCategoryType:@"2"];
...
{
    [self submitSupportWithCategoryType:@"2" withSupportView:sender];
...
{
    StoryComicNewsInfoFrame *frame = [self.storyArray lastObject];
